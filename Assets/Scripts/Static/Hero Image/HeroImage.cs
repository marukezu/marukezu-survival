...
// Essa classe armazena todos os dados do Hero durante uma "Run".
...
public static class HeroImage
{
    ...
    public static void PrepareHeroImage()
    {
        ...
        // Summons
        HeroImage_Summons.ResetSummons();
    }
    ...
    public static void ResetHeroImage()
    {
        ...
        // Reseta todas as Spells do jogo.
    ...
    public static void UpLevel()
    {
        ...
        GameManager.Instance.playerHero.heroTalents.AddTalentPoints(Hero_Talents.TALENTOS_POR_LEVEL);
    ...
    public static float GetHeroSpeed()
    {
        ...
        if (heroType != Hero.HeroType.None)
        {
            ...
            heroBase = heroData.baseStatus.heroBaseMovSpeed;
        }
        ...
        if (GameManager.Instance.playerHero != null)
        {
            ...
            Talents_PassoLigeiro = (Hero_Talents.GetBaseBuff(Hero_Talents.TalentType.VelMovimento) * GameManager.Instance.playerHero.heroTalents.GetLevel(Hero_Talents.TalentType.VelMovimento)) / 100;
    ...
    public static float GetHeroDamageBoost()
    {
        ...
        {
            Hero heroData = HerosList.heroes.FirstOrDefault(h => h.typeHero == heroType);
            heroBase = heroData.baseStatus.heroBaseDamagePercent;
        }
    ...
    public static float GetHeroCooldownReduction()
    {
        ...
        {
            Hero heroData = HerosList.heroes.FirstOrDefault(h => h.typeHero == heroType);
            heroBase = heroData.baseStatus.heroBaseCooldownPercent;
        }

        // Pega o bonus pelo nível do Talento (Apressado)
        if (GameManager.Instance.playerHero != null)
        {
            Talents_Apressado = Hero_Talents.GetBaseBuff(Hero_Talents.TalentType.TempoRecarga) * GameManager.Instance.playerHero.heroTalents.GetLevel(Hero_Talents.TalentType.TempoRecarga);
        }
    ...
    public static float GetHeroMaxHP()
    {
        ...
        {
            Hero heroData = HerosList.heroes.FirstOrDefault(h => h.typeHero == heroType);
            heroBase = heroData.baseStatus.heroBaseMaxHealth;
        }

        // Pega o bonus pelo nível do Talento (Pele de Ferro)
        if (GameManager.Instance.playerHero != null)
        {
            Talents_PeleFerro = (Hero_Talents.GetBaseBuff(Hero_Talents.TalentType.VidaMaxima) * GameManager.Instance.playerHero.heroTalents.GetLevel(Hero_Talents.TalentType.VidaMaxima)) / 100;
        }
    ...
    // ================================== GETS CHANCES HEROI ================================
    // ======================================================================================
    public static float GetHeroCriticalChance()
    {
        float heroBase = 5f, talent_Critical = 0f;

        // Pega o bonus pelo nível do Talento (Critical Chance)
        if (GameManager.Instance.playerHero != null)
        {
            Hero_Talents heroTalents = GameManager.Instance.playerHero.heroTalents;
            talent_Critical = Hero_Talents.GetBaseBuff(Hero_Talents.TalentType.ChanceCritica) * heroTalents.GetLevel(Hero_Talents.TalentType.ChanceCritica);
        }

        float somaFinal = heroBase + talent_Critical;

        return somaFinal;
    }

    public static float GetHeroImpalementChance()
    {
        float heroBase = 0f, talent_Impalement = 0f;

        // Pega o bonus pelo nível do Talento (Critical Chance)
        if (GameManager.Instance.playerHero != null)
        {
            Hero_Talents heroTalents = GameManager.Instance.playerHero.heroTalents;
            talent_Impalement = Hero_Talents.GetBaseBuff(Hero_Talents.TalentType.ChanceEmpalamento) * heroTalents.GetLevel(Hero_Talents.TalentType.ChanceEmpalamento);
        }

        float somaFinal = heroBase + talent_Impalement;

        return somaFinal;
    }

    // ======================================================================================
    // =============================== GETS MULTIPLICADORES HEROI ===========================
    // ======================================================================================
    public static float GetHeroCriticalMultiplier()
    {
        float heroBase = 120f, talent_Critical = 0f;

        // Pega o bonus pelo nível do Talento (Critical Chance)
        if (GameManager.Instance.playerHero != null)
        {
            Hero_Talents heroTalents = GameManager.Instance.playerHero.heroTalents;
            talent_Critical = Hero_Talents.GetBaseBuff(Hero_Talents.TalentType.MultCritico) * heroTalents.GetLevel(Hero_Talents.TalentType.MultCritico);
        }
